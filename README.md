...
- **频道未读提醒**：参考 `doc/channel-digest-push.md`，覆盖时间窗口、访问阈值、主动推送、被动拉取、测试推送及接收端校验程序。
- **子项目集成**：参考 `doc/sealchat-integration-v1.md`，按 README、API、service、model、protocol、pm 的真实代码结构梳理第一版接入文档。
...
### 用户密码重置（CLI）
可通过命令行直接读取当前配置对应数据库，列出平台管理员，或按用户名重置密码为 `123456`。

```bash
# 列出平台管理员用户
./sealchat_server --user-secret list

# 重置指定用户密码（支持平台管理员和普通用户）
./sealchat_server --user-secret reset --username alice --username bob

# 仅允许重置平台管理员，并跳过确认
./sealchat_server --user-secret reset --admin-only --username alice --yes
```

数据库连接优先级：
- `SEALCHAT_DSN` 环境变量
- `config.yaml` 中的 `dbUrl`
- 默认 `./data/chat.db`
